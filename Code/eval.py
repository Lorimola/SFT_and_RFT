...
def _init_llm(model_name):
    ...
    if _llm is None:
        _llm = Qwen2_5_VLForConditionalGeneration.from_pretrained(model_name, 
...
# check whether model is None and move it to device
def move_to(device):
    global _llm, _tokenizer
    if _llm is None:
...
# input user's instruction and get response from Qwen model
def get_qwen_response_low(user_query: str, screenshot: str, history_actions: list, low_instruction: str, args=None, model_path: str = "/home/test/test03/models/Qwen2.5-VL-7B-Instruct") -> tuple:
    ...
    global _llm,_tokenizer
    try:
        # process the size of input image
        ...
        # initialize mobile device interface, which can use a touchscreen to interact with a mobile device and take screenshot
        ...
        # build message, which is a list of dictionary
        prompt = NousFnCallPrompt()
        # add user message and system message
